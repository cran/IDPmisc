"inf.omit" <-
function(x){
  ## omits all NA and infinite values in vectors, data.frames and matrices
  ## ... Further arguments for na.omit
  ## Returns all other objects unchanged and prints a warning.
  ...
  if (is.data.frame(x)) {
    ...
    warning("\n'x' is neither a vector, nor a data.frame nor a matrix. 'x' is returned unchanged")
    return(x)
  }
  return(x)
}## inf.omit
